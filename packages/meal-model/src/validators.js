...
export function validateWallClockTime(input, fieldPrefix = '') {
    const errors = [];
    ...
    return { ok: true, value: { hour: hour, minute: minute } };
}
...
function validateDish(input, prefix) {
    const errors = [];
    if (typeof input !== 'object' || input === null || Array.isArray(input)) {
...
export function validateMealPlan(input) {
    const errors = [];
    if (typeof input !== 'object' || input === null || Array.isArray(input)) {
