...
function toMinutes(time: WallClockTime): number {
  return time.hour * 60 + time.minute;
}

/** Converts total minutes from midnight (mod 1440) to WallClockTime. */
function fromMinutes(totalMinutes: number): WallClockTime {
 ...
 */
function subtractMinutes(time: WallClockTime, minutes: number): WallClockTime {
 ...
 */
function addMinutes(time: WallClockTime, minutes: number): WallClockTime {
  return fromMinutes(toMinutes(time) + minutes);
}
 ...
 */
function formatWallClockTime(time: WallClockTime): string {
...
export const timingEngine: TimingEngine = {
  subtractMinutes,
  addMinutes,
  differenceMinutes,
  formatWallClockTime,
